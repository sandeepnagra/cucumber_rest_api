...
As a  developer
I want to be able to execute functional tests against my API using Mimic for POST method
...
Scenario Outline: Display 200 status by passing payload using POST
	Given I set the test server in JSON format with the following values:
	| uri                               | json                                         | request_type | statuscodeno |
	| /api/coffeefinder/location/lookup | {"Stores": [{"MandS","Waitrose","Sainsbury"} | post         | 200          |
	When I send a POST request to "<uri>" with the following:
	| lookup | store |
	Then the response status should be "<statuscodeno>"
	Examples:
	| uri                               | json | request_type | statuscodeno |
	| /api/coffeefinder/location/lookup |      | post         | 200          |
